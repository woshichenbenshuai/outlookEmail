...
# 登录密码配置（可以修改为你想要的密码）
LOGIN_USERNAME = os.getenv("LOGIN_USERNAME", "admin").strip() or "admin"
...
def get_encryption_key() -> bytes:
    ...
    """
    base_secret_key = str(app.secret_key or "").strip()
    if not base_secret_key:
    ...
    key = base64.urlsafe_b64encode(kdf.derive(base_secret_key.encode()))
    return key
...
def init_db():
    ...
    if existing_login_username:
        login_username_value = (existing_login_username[0] or '').strip()
        if not login_username_value:
            cursor.execute(
        ...
        cursor.execute(
            "INSERT INTO settings (key, value) VALUES ('login_username', ?)",
            (LOGIN_USERNAME,)
        )
