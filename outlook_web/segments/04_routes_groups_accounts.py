...
def login():
    ...
    if request.method == 'POST':
        try:
            ...
            username_ok = secrets.compare_digest(username, stored_username)
...
def api_update_account(account_id):
    """更新账号"""
    if 'api_update_account_v2' in globals():
...
def api_update_account_status(account_id: int, status: str):
    ...
    if normalized_status not in ('active', 'inactive'):
        return jsonify({'success': False, 'error': '状态无效，仅支持 active / inactive'}), 400
